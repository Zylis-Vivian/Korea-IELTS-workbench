...
- **iPhone / iPad PWA**：支持添加到主屏幕、独立 Web App 窗口、离线应用壳、弱网缓存、版本更新提示和移动端安全区适配。
- **Vercel Preview**：GitHub 分支提交会自动生成可在真实 iPhone / iPad 上验证的预览地址。
...
1. 在 iPhone / iPad 的 Safari 打开工作台，分享 → 添加到主屏幕 → 打开「作为 Web App」；固定域名后再长期积累学习记录。
2. 第一次使用先打开「今日任务」，按顺序处理到期内容，不要一开始只刷新词。
3. 看到句子后先盲听，再显示中文；能听懂后进入「影子跟读」，用 0.75x 或 0.9x 练两遍，再用 1x 录音回听。
4. 每张卡选择真实掌握程度：「重来」代表现在仍不会，「困难」代表需要短间隔复习，「正常」代表按计划复习，「简单」才会拉长间隔。
5. 韩语初学阶段可以打开罗马音；熟悉韩文后建议关闭罗马音，只保留韩文和实际发音。
6. 雅思听力先用示例文本熟悉流程，再使用「听写练习」和「影子跟读」巩固；如需加入自己的音频，请只使用有授权的本地资源，不要把受版权保护的文件上传到公共仓库。
...
- [x] 增加 iPhone / iPad PWA：manifest、主屏图标、Service Worker、离线应用壳、更新提示、安全区和移动端主导航。
- [x] 增加 Vercel Express 入口与部署配置，支持生产构建后的前端和 `/api/tts` 同源运行。
